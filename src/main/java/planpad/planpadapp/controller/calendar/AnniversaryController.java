...
import planpad.planpadapp.dto.api.calendar.AnniversaryResponseWrapper;
...
public class AnniversaryController {
    ...
    @Operation(summary = "기념일 상세 조회", description = "기념일 상세 정보를 조회합니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "기념일 상세 조회 성공", content = @Content(mediaType = "application/json", schema = @Schema(implementation = AnniversaryResponseWrapper.class))),
            @ApiResponse(responseCode = "400", description = "기념일 상세 조회 실패", content = @Content(mediaType = "application/json", schema = @Schema(implementation = OnlyMessageResponseDto.class)))
    })
    public ResponseEntity<Object> getAnniversary(@RequestHeader("Authorization") String bearerToken, @PathVariable("id") Long id) {

        try {
            User user = userService.getUserByBearerToken(bearerToken);
            AnniversaryResponse anniversary = anniversaryService.getAnniversary(user, id);

            return ResponseEntity.ok(new AnniversaryResponseWrapper(anniversary, "기념일 상세 조회에 성공하였습니다."));

        } catch (Exception e) {
            return ResponseEntity.badRequest().body(new OnlyMessageResponseDto("기념일 상세 조회에 실패하였습니다."));
        }
    }
